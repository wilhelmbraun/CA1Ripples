...
"""
###
#from ripple_generation_new.ipynb
#Generates Fig.3
...
def spike_generation(plot_indicator, sharp_wave_amplitude, n_excited):
    ...
    #print('maximal number of neurons that would get negative drive', len(np.where(amplitude<=0)[0]))
    #exclude values that are below zero
    ...
    if plotting:
        ...
        ax0.annotate('A', xy=(-0.12, 1.0), xycoords='axes fraction', fontsize= 24.0) #36.0 #12.0
        ...
        fig.savefig("/home/wilhelm/Schreibtisch/Fig3A.png", bbox_inches='tight', dpi= 400)
        pl.show()

        #exit()

        fig = pl.figure(1, figsize = (7.5, 1.5), dpi = 400)
        ...
        pl.tight_layout(pad = 1.0)
        fig.savefig("/home/wilhelm/Schreibtisch/Fig3B.png", bbox_inches='tight', dpi = 400)
        pl.show()
...
if single_runs:
    ...
    for k in np.arange(n_reps):
        #seed(1)
        result = spike_generation(plot_indicator = True, sharp_wave_amplitude = 65.0, n_excited = 400)
